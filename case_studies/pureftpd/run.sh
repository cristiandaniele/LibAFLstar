...
timeout "${TIMEOUT}" \
  cargo run --release --bin "libaflstar-ftp-${BIN}" -- \
    --in-dir /corpus \
    --out-dir /output_pureftpd \
    --target-port 21021 \
    --loops "${LOOPS}" \
    -t 300 \
    /pure-ftpd/src/pure-ftpd -S 127.0.0.1,21021 -N --dontresolve --ipv4only
rc=$?
...
cp /output_pureftpd/stats.json "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
cp /output_pureftpd/total_stats_info.txt "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
cp -r /output_pureftpd/crashes "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
