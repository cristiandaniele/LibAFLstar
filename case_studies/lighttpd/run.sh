...
# Run
timeout "${TIMEOUT}" \
  cargo run --release --bin "libaflstar-http-${BIN}" -- \
    --in-dir /corpus \
    --out-dir /output_lighttpd \
    --target-port 8080 \
    --loops "${LOOPS}" \
    -t 300 \
...
cp /output_lighttpd/stats.json "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
cp /output_lighttpd/total_stats_info.txt "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
cp -r /output_lighttpd/crashes "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
