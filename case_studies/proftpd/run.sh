...
timeout "${TIMEOUT}" \
  cargo run --release --bin "libaflstar-ftp-${BIN}" -- \
    --in-dir /corpus \
    --out-dir /output_proftpd \
    --target-port 21001 \
    --loops "${LOOPS}" \
    -t 300 \
    /proftpd/proftpd -c /basic.conf -X -n
rc=$?
...
cp /output_proftpd/stats.json "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
cp /output_proftpd/total_stats_info.txt "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
cp -r /output_proftpd/crashes "/results/${BIN}-${LOOPS}-${TIMEOUT}/"
